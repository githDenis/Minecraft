#include "Texture.h"

Texture::Texture(const char* filePath)
{
	SetImage(filePath);
}

Texture::~Texture()
{
	glDeleteTextures(1, &texture);
}

void Texture::Create()
{
	glGenTextures(1, &texture);
	glBindTexture(GL_TEXTURE_2D, texture);

	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);

	glBindTexture(GL_TEXTURE_2D, 0);
}

void Texture::SetImage(const char* filePath)
{
	glBindTexture(GL_TEXTURE_2D, texture);

	int width, height, nChannels;
	unsigned char* imageBuf = stbi_load(filePath, &width, &height, &nChannels, 4);

	if (imageBuf)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0,
			GL_RGBA, GL_UNSIGNED_BYTE, imageBuf);
		glGenerateMipmap(GL_TEXTURE_2D);
	}
	else
	{
		std::cout << stbi_failure_reason() << '\n';
	}
	glBindTexture(GL_TEXTURE_2D, 0);
	stbi_image_free(imageBuf);
}

UV Texture::GetUV(int imageID, int xDivider, int yDivider) noexcept
{
	float imageWidth = 1.0 / static_cast<float>(xDivider);
	float imageHeight = 1.0 / static_cast<float>(yDivider);

	int x = imageID % xDivider;
	int y = imageID / xDivider;

	const float PADDING = 0.001f;

	UV uv;
	uv.u0 = static_cast<float>(x) * imageWidth;
	uv.v0 = static_cast<float>(y) * imageHeight;
	uv.u1 = uv.u0 + imageWidth;
	uv.v1 = uv.v0 + imageHeight;

	uv.u0 += PADDING;
	uv.v0 += PADDING;
	uv.u1 -= PADDING;
	uv.v1 -= PADDING;
	return uv;
}